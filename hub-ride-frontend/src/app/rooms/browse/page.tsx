...
import { Clock3, MapPinned, Route, Send, UsersRound } from "lucide-react";
        ...
        <div className="rounded-2xl border bg-card p-5">
            <div className="flex items-start justify-between gap-3">
                ...
                <Badge variant="secondary">
                    {formatCountdown(room.countdownSec)}
                </Badge>
            </div>
...
export default function BrowseRoomsPage() {
    ...
    const joinMutation = useMutation({
        mutationFn: (roomId: string) =>
            joinRoom(roomId, currentUser.id).then(() => roomId),
        onSuccess: (roomId) => {
            ...
            <section className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                ...
                >
                    ...
                    <p className="mt-2 text-sm leading-6 text-muted-foreground">
                        Choose a matching route so the backend can find nearby
                        rooms with H3 k-ring matching.
                    </p>
                ...
                <div className="mt-6">
                    {!canSearch ? (
                        <div className="rounded-2xl border border-dashed bg-card p-8 text-center text-sm text-muted-foreground">
                            Choose an origin and destination to start browsing
                            rooms.
                        </div>
                    ) : isFetching ? (
                        ...
                        <div className="rounded-2xl border bg-card p-8 text-center">
                            ...
                            <p className="mt-1 text-sm text-muted-foreground">
                                Create a new room so others can join the same
                                route.
                            </p>
